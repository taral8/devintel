...
import { findSimilarProjects, computeApprovalScore } from "./scoring";
import type { DADetail, StatsResponse } from "./types";

const VALID_COUNCILS = ["Parramatta", "Blacktown", "Hornsby"];

export function getStatsData(): StatsResponse {
  ...
  return { total_das, overall_approval_rate, councils };
}

export function getDAData(id: string): DADetail | null {
  const da = MOCK_DAS.find((d) => d.id === id);
  if (!da) return null;

  const similar_projects = findSimilarProjects(da.id);
  const approval_score = computeApprovalScore(da.id);

  return { ...da, similar_projects, approval_score };
}
