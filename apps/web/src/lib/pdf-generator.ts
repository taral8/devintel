...
export async function generateDAReportPDF(da: DADetail): Promise<Uint8Array> {
  ...
  // ── Metadata ──────────────────────────────────────────────────
  doc.setTitle(`DA Risk Analysis Report — ${da.id.toUpperCase()}`);
  doc.setAuthor("CivroDA — Approval Risk Intelligence");
  ...
  function drawPageFooter(p: PDFPage, font: PDFFont, pageNum: number) {
    ...
    p.drawText("CivroDA — Approval Risk Intelligence", {
      x: MARGIN,
      y: footerY,
      size: 7,
      font,
  ...
  page.drawText("RISK ANALYSIS REPORT", {
    x: MARGIN,
    y: PAGE_H - 48,
    size: 14,
    font: fontBold,
  ...
  if (da.approval_score > 0) {
    ...
    const scoreLabel = "Approval Risk Score";
  ...
  if (da.similar_projects && da.similar_projects.length > 0) {
    ...
    page.drawText("COMPARABLE PRECEDENTS IN COUNCIL AREA", {
      x: MARGIN,
      y,
      size: 12,
      font: fontBold,
  ...
  if (da.approval_score > 0) {
    ...
    page.drawText("APPROVAL RISK ANALYSIS", {
      x: MARGIN,
      y,
      size: 12,
      font: fontBold,
