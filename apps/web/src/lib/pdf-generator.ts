...
// ── Colour palette ──────────────────────────────────────────────
const BRAND_NAVY = rgb(0, 0.2, 0.4); // #003366
...
const GREEN = rgb(0.05, 0.47, 0.25);
...
export async function generateDAReportPDF(da: DADetail): Promise<Uint8Array> {
  ...
  doc.setAuthor("DevIntel");
  doc.setSubject(`Development Application ${da.id} — ${da.address}`);
  doc.setCreator("DevIntel (https://devintel.site)");
  ...
  function drawPageFooter(p: PDFPage, font: PDFFont, pageNum: number) {
    ...
    p.drawText("DevIntel — Development Application Intelligence", {
      x: MARGIN,
      y: footerY,
      size: 7,
      font,
  ...
  page.drawText("devintel.site", {
    x: PAGE_W - MARGIN - fontRegular.widthOfTextAtSize("devintel.site", 8),
    y: PAGE_H - 50,
    size: 8,
    font: fontRegular,
